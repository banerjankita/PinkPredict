...
export default function Home() {
  return (
    <div className="relative w-screen h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white overflow-x-hidden">
      ...
      <div className="relative flex flex-col items-start justify-start w-full md:w-3/5 mx-auto px-6 text-left z-10 pt-20 h-full overflow-y-auto">
        ...
        {/* Heading */}
        <h1 className="text-4xl md:text-6xl font-extrabold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-pink-500 via-pink-400 to-pink-300 drop-shadow-xl text-center w-full tracking-wide uppercase leading-snug">
          <span className="text-2xl text-gray-300 block">Welcome to</span>
          <span className="text-pink-400">PinkPredict</span>
        </h1>

        <p className="text-base md:text-xl font-medium text-gray-300 mb-6 max-w-2xl text-center md:text-right w-full italic">
  Empowering early detection with AI-driven breast cancer prediction.
</p>


        {/* What is PinkPredict? */}
        <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-1">What is PinkPredict?</h2>
        ...
        {/* Who is it for? */}
        <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-6">Who is it for?</h2>
        <p className="text-base md:text-lg  text-gray-300 mt-4 max-w-2xl">
        ...
        <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-6">What does PinkPredict do?</h2>
        <p className="text-base md:text-lg  text-gray-300 mt-4 max-w-2xl mb-10">
  ...
  <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-0 mb-0">Our Mission</h2>
...
<div id="team" className="mt-6 max-w-4xl w-full">
  <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mb-0">Our Team</h2>

  {/* Text and Image Container */}
  <div className="w-full flex flex-col items-center mt-6">
    
    {/* First Paragraph */}
    <p className="text-base md:text-lg text-gray-300 w-full text-justify">
...
<div id="how-it-works" className="mt-10 max-w-4xl w-full mx-auto px-4">
  <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-6">How It Works</h2>

  <p className="text-base md:text-lg text-gray-300 mt-6 text-justify">
  ...
  <ul className="text-base md:text-lg text-gray-300 mt-4 space-y-4">
    {[
      ["Feature Extraction", "The system takes 31 input values, including radius mean, texture mean, perimeter mean, smoothness mean, and other characteristics of the cell nuclei."],
      ["Data Preprocessing", "The inputs are normalized to ensure consistency and eliminate any bias due to scale variations."],
      ["Feature Analysis", "Our machine learning model identifies key patterns and relationships among these features that indicate malignancy or benignity."],
      ["Model Prediction", "The processed data is fed into a trained classification model, which calculates the probability of the tumor being cancerous."],
      ["Thresholding & Decision Making", "If the probability surpasses a certain threshold, the tumor is classified as malignant; otherwise, it is considered benign."],
      ["Result Interpretation", "The model outputs a clear result, along with a probability score, to help individuals understand their risk level."],
      ["Encouraging Medical Consultation", "While our system provides a reliable prediction, we strongly recommend consulting a medical professional for further evaluation and screening."]
    ].map(([title, desc], idx) => (
      <li key={idx} className="flex items-start">
        <span className="text-pink-400 mr-2">•</span>
        <span>
          <span className="text-pink-400 font-semibold">{title}:</span>{' '}
          <span className="italic text-gray-300">{desc}</span>
        </span>
      </li>
    ))}
  </ul>
...
<div id="faqs" className="mt-10 max-w-4xl w-full mx-auto px-4">
  <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-6">Frequently Asked Questions (FAQs)</h2>

  <div className="mt-6 space-y-6">
    {[
      ["1. What is the purpose of this breast cancer prediction model?", "This AI-powered model helps predict whether a breast tumor is malignant (cancerous) or benign (non-cancerous) based on medical input features, assisting in early detection."],
      ["2. How accurate is the prediction?", "The model is trained on a high-quality dataset and achieves high accuracy. However, it should not replace professional medical diagnosis and consultation."],
      ["3. What input values does the model require?", "The model takes 31 medical input features, such as radius mean, texture mean, perimeter mean, and smoothness mean, extracted from cell nuclei."],
      ["4. Is this model suitable for self-diagnosis?", "No, this model is designed to assist medical professionals. A final diagnosis should always be made by a doctor after thorough evaluation."],
      ["5. Can I use this model for real-time predictions?", "Yes, the model is capable of making real-time predictions when provided with the required medical data, ensuring quick assessments."],
      ["6. What should I do if the model predicts a malignant tumor?", "If the model predicts malignancy, consult a healthcare professional immediately for further tests and confirmation. Early detection is crucial for effective treatment."]
    ].map(([q, a], idx) => (
      <div key={idx}>
        <h3 className="text-xl text-pink-400">{q}</h3>
...
<div id="terms" className="mt-10 max-w-4xl w-full mx-auto px-4">
  <h2 className="text-2xl md:text-3xl font-extralight text-pink-400 mt-6">Terms of Use</h2>

  <ul className="mt-6 space-y-4 list-none">
    {[
      ["No Medical Advice", "This AI model is for informational and research purposes only. It does not provide medical advice, diagnosis, or treatment. Always consult a qualified healthcare professional for medical concerns."],
      ["Accuracy and Limitations", "While this model is trained for high accuracy, predictions may not always be 100% correct. The system should not be solely relied upon for critical health decisions."],
      ["Data Privacy", "We do not store or share any user input data. However, users are responsible for ensuring that any data entered complies with privacy and security regulations."],
      ["Personal Responsibility", "Users agree to use this system at their own risk. We are not responsible for any consequences arising from the use or misuse of this tool."],
      ["Modification of Terms", "We reserve the right to modify or update these terms at any time without prior notice. Continued use of the system implies acceptance of the updated terms."],
      ["Contact Information", "If you have any questions or concerns regarding these terms, please contact us through our official channels."]
    ].map(([title, desc], idx) => (
      <li key={idx} className="flex items-start">
        <span className="text-pink-400 mr-2">•</span>
